            ...
            <div className="w-24 md:w-32 h-32 flex-shrink-0 relative">
               <div className="absolute bottom-0 left-0 md:-left-4 z-20 w-40 md:w-52 max-w-none pointer-events-none">
                  <img 
                    ...
                    style={{ maxHeight: '140%' }}
                    onError={(e) => { e.currentTarget.style.display='none'; e.currentTarget.parentElement!.innerHTML='<div class="h-full flex items-center justify-center"><i class="fas fa-bullhorn text-4xl text-warning transform -rotate-12"></i></div>'; }} 
                  />
               </div>
              ...
              >
                ...
                <div className="absolute inset-0 bg-slate-900">
                   <img 
                     src={item.image} 
                     alt={item.title} 
                     className="w-full h-full object-cover opacity-80 group-hover/card:opacity-60 group-hover/card:scale-110 transition-all duration-700"
                   />
                </div>
                ...
                {/* Advanced Stat Background Pattern */}
                {item.isAdvanced && (
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-end">
                   ...
                   <div className="mb-4">
                      ...
                      {/* Quote section if available, otherwise summary */}
                      {item.quote ? (
                   ...
                   <div className="flex items-center gap-3">
                      <div className={`w-full py-3 backdrop-blur-sm border rounded-lg text-center font-bold text-white text-sm transition-all flex items-center justify-center gap-2 ${item.isAdvanced ? 'bg-yellow-600/20 border-yellow-400/50 group-hover/card:bg-yellow-500 group-hover/card:border-yellow-500 text-yellow-100' : 'bg-white/10 border-white/20 group-hover/card:bg-accent group-hover/card:border-accent'}`}>
                         {item.isAdvanced ? 'View Analysis' : 'Search Story'} <i className="fas fa-search text-xs transform group-hover/card:scale-110 transition-transform"></i>
                      </div>
                   </div>
                </div>
              </a>
                 ...
                 >
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                       <div className={`w-12 h-12 rounded-lg flex-shrink-0 overflow-hidden relative ${item.isAdvanced ? 'border border-yellow-400' : ''}`}>
                          ...
                          {item.isAdvanced && <div className="absolute inset-0 bg-yellow-500/20"></div>}
                       </div>
